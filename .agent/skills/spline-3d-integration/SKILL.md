...
### 5. Fullscreen Background with Native Mouse Tracking

Place a Spline scene as a **full-screen background** behind text/UI content, while keeping the robot's/scene's internal mouse tracking alive — and without breaking clicks on buttons or links.

#### The Problem

If you put the Spline canvas behind other content (via `z-index`), two things break:
1. **`pointer-events: none` on Spline** → the scene can't detect mouse movement → no internal tracking
2. **`pointer-events: auto` on Spline** → it blocks all clicks on buttons/links on top of it

#### ❌ What Does NOT Work

```tsx
// WRONG — Spline empfängt keine Maus-Events mehr
<div style={{ pointerEvents: 'none', position: 'absolute', inset: 0, zIndex: 0 }}>
  <Spline scene="..." style={{ pointerEvents: 'none' }} />
</div>

// WRONG — CSS-Transform auf dem Wrapper bewegt den ganzen Container, nicht den Roboter intern
const containerRef = useRef();
// mousemove → containerRef.current.style.transform = `translate(${x}px, ${y}px)`
// Ergebnis: Der Layout-Block verschiebt sich, nicht die 3D-Szene selbst

// WRONG — Synthetische Events per dispatchEvent funktionieren nicht zuverlässig mit Spline
canvas.dispatchEvent(new MouseEvent('mousemove', { clientX, clientY }));
```

#### ✅ The Correct Solution: Pointer-Events Layering

The key insight: **Let Spline receive mouse events naturally** by making the overlaying content layer transparent to pointer events — but selectively re-enable `pointer-events` only on interactive elements.

```
Layer stack:
  z-index: 20 → Content grid     (pointer-events: NONE  → mouse passes through)
                  └── <button>   (pointer-events: AUTO  → still clickable ✅)
                  └── <a>/<Link> (pointer-events: AUTO  → still clickable ✅)
  z-index:  0 → Spline canvas    (pointer-events: AUTO  → gets all mouse events ✅)
```

**`RobotCanvas.tsx` — Spline bekommt volle Pointer-Events:**
  ...
  return (
    <div
      style={{
        ...
        // NO pointerEvents: 'none' here! Spline must receive mouse events.
      }}
    >
      <Spline
        style={{ width: "100%", height: "100%" }}
        // also NO pointerEvents: 'none' on the Spline component itself
...
**`page.tsx` — Hero-Bereich mit korrekter Pointer-Events-Strategie:**

```tsx
<main className="relative" style={{ overflow: 'hidden' }}>

  {/* Spline background — z-index: 0, pointer-events: auto (default) */}
  <div className="absolute inset-0 z-0">
    <RobotCanvas />
  </div>

  {/* Content layer — pointer-events: none so mouse reaches Spline */}
  <div className="relative z-20 grid ..." style={{ pointerEvents: 'none' }}>

    <h2>Mein Titel</h2>  {/* non-interactive, keine pointer-events nötig */}

    {/* Interactive elements get pointer-events: auto back explicitly */}
    <button style={{ pointerEvents: 'auto' }}>Jetzt entdecken</button>
    <Link href="#" style={{ pointerEvents: 'auto' }}>Mehr erfahren</Link>
...
- Mouse moves over the page → hits the content grid (z-index: 20) → `pointer-events: none` → events pass through
- Events land on the Spline canvas below → **scene reacts to mouse internally** ✅
- User clicks a button/link → those have `pointer-events: auto` re-applied → **clicks work** ✅
- No JavaScript hacks, no `dispatchEvent`, no CSS transforms needed

> **Key Rule:** Never set `pointer-events: none` on the Spline component or its wrapper if you want the scene to react to mouse movement. Instead, set it on the overlaying content and selectively restore it on interactive children.

See [`background-spline-hero.tsx`](examples/background-spline-hero.tsx) for the full production example.
